...
def simulateRR(Processes, num_cpus, dead_condition, time_slice):
	...
	while True:
		if ((not Processes.empty()) and not cpus.full()):
			job=cpus.addJob(Processes.get())
			if job!=None:
				print "WE TOOK A JOB OFF THAT WE WEREN'T SUPPOSED TO"
				Processes.put(job)
				break
		else:
			break
	...
	while True:
		...
		for i in range(0,len(ready_processes)):
			#we need to set the RRtime
			
			Processes.put(ready_processes[i])		
		...
		#now we check whether any jobs have gone over the RR timeslice
		for process in range(0, len(cpus.jobs)):
			if (cpus.jobs[process].getRRtime() >= time_slice):
				#kick the process off and add a new one
				old_job=cpus.jobs.pop(process)
				Processes.put(old_job)
				new_job=Processes.get()
				print "[time %dms] Context switch (swapping out process ID %d for process ID %d)"%(time, old_job.ID(), new_job.ID())
				if not (old_job.ID()==new_job.ID()):
					if old_job.isIOBound():
						print "[time %dms] Interacive process ID"%time, old_job.ID(), "entered the ready queue (requires","%dms CPU time)"%old_job.burstTime()
					else:
						print "[time %dms] CPU-bound process ID"%time, old_job.ID(), "entered the ready queue (requires","%dms CPU time)"%old_job.burstTime()
				new_job.setRRtime(-2)
				new_job.CPUBurst+=2
